...
unordered_map<int,string> error_info =
        {
                ...
                {9,"invalid argument number for compare, "},
...
public:
    ...
    int scope_num;
    int line_num;
    enum {VARIABLE, FUNCTION, STRUCTURE, EXPRESSION, ARRAY} attribute;
    vector<Type*> args;
    ...
    SymbolElement(string value, Type * type, int scope_num, int line_num, string attr, vector<Type*>* vec): value(value), type(type), line_num(line_num), scope_num(scope_num) {
        if (attr == "VAR"){
            this->attribute = VARIABLE;
        }else if (attr == "FUNC"){
            this->attribute = FUNCTION;
...
void extDefEntry(ast_node* node){
    ...
    if(node->children_num >= 2){
        ...
        if(child->name == "FunDec" && node->children_num == 3){
            ...
            if(child->children_num >2 && node->children[2]->name == "VarList" ){
                symbolTable.scope--;
            }
...
//void Stmt(syntaxNode* node, Type* retTpy) {
//    syntaxNode* child = node->children[0];
//    if (child->name == "Exp") Exp(child); // Exp SEMI
//    else if (child->name == "CompSt") CompSt(child, retTpy); // CompSt
//    else if (child->name == "RETURN") {
//        // RETURN Exp SEMI
//        TypeExt* expInfo = Exp(node->children[1]);
//        if (expInfo->type != NULL && !(*retTpy == *(expInfo->type))) {
//            semantic_error(8, node->children[1]->lineNum, "incompatiable return type");
//        }
//    }
//    else if (child->name == "IF") {
//        // IF LP Exp RP Stmt (ELSE Stmt)
//        TypeExt* expInfo = Exp(node->children[2]);
//        Stmt(node->children[4], retTpy);
//        if (node->children.size() > 6) Stmt(node->children[6], retTpy);
//    }
...
void stmtEntry(ast_node* node,string func_id){
    ...
    if (node->children_num == 2 && node->children[0]->name == "Exp"){
        ...
        if(expType != nullptr){
           ...
           if(info->type == nullptr || info->type->name != expType->name){
...
void decEntry(ast_node* node, Type* spec_type,vector<Type*> * vec_po){
    ...
    if(node->children_num == 3){
        ...
        if(type != nullptr && type_exp != nullptr && type_exp->name != type->name){
...
Type * checkFunc(ast_node* node, vector<Type*>* args){
    ...
    if(args != nullptr){
        if(info_FUNC->args.size() != args->size()){
            reportError(9,node->line_num,"expect "+ to_string(info_FUNC->args.size())+", got "+ to_string(args->size()));
            //cout << "can shu bu dui" << endl;
        }
    }
...
Type* ExpressionEntry(ast_node* node){
    ...
    if(node->children_num == 1 && node->children[0]->name == "ID"){
        ...
        if(node->children_num == 3 && node->children[1]->name == "ASSIGN"){
            ...
            if(right_exp_type == nullptr){
                reportError(5,node->line_num,"");
                return nullptr;
            }

            if (right_exp_type!= nullptr && left_exp_type!= nullptr ){
                string co_array = "Array";
                string namel = left_exp_type->name;
                string namer = right_exp_type->name;
                if( (left_exp_type->name).compare(0,co_array.size(),co_array) == 0){
                    namel = left_exp_type->name.substr(6);
                }
                if( (right_exp_type->name).compare(0,co_array.size(),co_array) == 0){
                    namer = left_exp_type->name.substr(6);
                }
                if(namel != namer){
                    reportError(5,node->line_num,"");
                }
            ...
            if(left_exp_type != nullptr){
                ...
                if ((left_exp_type->name).compare(0,co_array.size(),co_array) == 0) {
                    lname = left_exp_type->name.substr(6);
                }
                if(lname != "Structure"){
                    reportError(13,node->line_num,"");
                    return nullptr;
                }
                if (lname == "Structure" && (left_exp_type->name).compare(0,co_array.size(),co_array) == 0 ) {
                    ArrayType * tmp_po = dynamic_cast<ArrayType*>(left_exp_type);
                    stType = dynamic_cast<StructureType*>(tmp_po->base);
                }else if (lname == "Structure"){
            ...
            string rtype = right_exp_type->name;
            if ((right_exp_type->name).compare(0,co_array.size(),co_array) == 0){
                rtype = right_exp_type->name.substr(6);
            }

            if(rtype != "Primitive_int"){
...
Type* structSpecifierEntry(ast_node* node){
    ...
    if(node->children_num == 5){
        ...
        SymbolElement* element = new SymbolElement(node->value, structType, symbolTable.scope, node->line_num, "STRUCT");
...
pair<string, SymbolElement*> createTableInfo(ast_node* node, Type *type, string attr, vector<Type*>* vec){
    ...
    if (attr == "VAR"){
        ...
        D( cerr << "lineno: " << __LINE__ << " node value : " << node->value << endl;)
        if(vec != nullptr){
            SymbolElement * element = new SymbolElement(node->value, type, symbolTable.scope, node->line_num, "FUNC",vec);
